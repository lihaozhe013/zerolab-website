...
function Destination({ href, className, children }: {
  href: string;
  className: string;
  children: ReactNode;
}) {
...
export default function ProductScrollShowcase({ items }: ProductScrollShowcaseProps) {
  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      const overview = section.querySelector<HTMLElement>('[data-product-overview]');
      ...
      const mediaTrack = section.querySelector<HTMLElement>('[data-product-media-track]');
      const copyViewport = section.querySelector<HTMLElement>('[data-product-copy-viewport]');
      const copyTrack = section.querySelector<HTMLElement>('[data-product-copy-track]');
      ...
      timeline
        ...
        .to(stackingCards, {
          x: (_index, target) => cards[0].offsetLeft - (target as HTMLElement).offsetLeft,
          y: (_index, target) => cards[0].offsetTop - (target as HTMLElement).offsetTop,
          duration: 0.72,
          stagger: 0.08,
        }, 0.95)
  ...
  if (useStaticLayout) {
    return (
      <Box component="section" className="bg-[#07090c] px-6 pb-28 pt-12 text-white">
        <Box className="mx-auto max-w-[1280px]">
          <Box className="grid grid-cols-2 gap-3">
            {items.map((item) => (
              <Box key={item.title} className="overflow-hidden bg-[#111419]">
                <Box component="img" src={item.image} alt={item.title} className="aspect-[4/5] w-full object-cover" />
              </Box>
            ))}
          </Box>
          ...
          <Box className="mt-24 space-y-28">
            {items.map((item) => (
              <Destination key={item.title} href={item.href} className="group block">
                <Box className="overflow-hidden bg-[#111419]">
                  <Box component="img" src={item.image} alt={item.title} className="aspect-[4/3] w-full object-cover" />
                </Box>
                <Typography
                  component="h2"
                  sx={{ fontSize: 32, fontWeight: 600, lineHeight: 1.05, letterSpacing: '-0.04em' }}
                  className="mt-7"
                >
                  {item.title}
                </Typography>
  ...
  return (
    <Box
      ref={sectionRef}
      component="section"
      className="relative h-[600dvh] bg-[#07090c] text-white"
    >
      <Box className="sticky top-0 min-h-[100dvh] overflow-hidden">
        ...
        >
          <Box className="mx-auto grid w-full max-w-[1280px] grid-flow-dense grid-cols-4 gap-5">
            {items.map((item, index) => (
              <Box
                key={item.title}
                data-product-card
                className={index % 2 === 1 ? 'relative mt-12 overflow-hidden bg-[#111419]' : 'relative overflow-hidden bg-[#111419]'}
              >
                <Box
                  component="img"
                  src={item.image}
        ...
        >
          <Box data-product-media-track className="flex h-full will-change-transform">
            {stageItems.map((item) => (
              <Destination
                key={item.title}
                href={item.href}
              ...
              >
                <Box
                  ...
                  src={item.image}
                  alt={item.title}
                  className="absolute inset-0 h-full w-full object-cover transition-transform duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-[1.025]"
                />
